...
//		sellTower, upgradeTower; 
	...
	public void draw(Graphics g) {
		...
		if (playing.isGamePaused()) {
			...
			g.drawString("Juego pausado!", 110, 790);
		}
	...
	private void drawTowerCost(Graphics g) {
		...
		g.drawString("Precio: " + getTowerCostCost() + "g", 285, 695);

		// Show this if player lacks gold for the selected tower.
		if (isTowerCostMoreThanCurrentGold()) {
			g.setColor(Color.RED);
			g.drawString("Cantidad de dinero insuficiente", 270, 725);
	...
	private void drawGoldAmount(Graphics g) {
		g.drawString("Oro: " + gold + "G", 110, 725);
	...
	private void drawDisplayedTower(Graphics g) {
		if (displayedTower != null) {
			...
			if (sellTower.isMouseOver()) {
				g.setColor(Color.red);
				g.drawString("Vender por: " + getSellAmount(displayedTower) + "G", 480, 695);
			} else if (upgradeTower.isMouseOver() && gold >= getUpgradeAmount(displayedTower)) {
				g.setColor(Color.blue);
				g.drawString("Mejorar por: " + getUpgradeAmount(displayedTower) + "G", 480, 695);
			}
